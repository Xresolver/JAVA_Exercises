...
public class Enhanced_Heuristic_Approache 
{
...
//		System.out.println();
	...
	private static void decide()
	{
		...
		// save minimum possible square.
		int minPossibleRow = 0;
		int minPossibleColumn = 0;
		...
		for(int i = 0; i < POSSIBLE_MOVE_NUMBER; i++)
		{
			...
			{	
														...
														[currentColumn + horizontal[i]])
				{
					...
					minPossibleRow = currentRow + vertical[i];
					minPossibleColumn = currentColumn + horizontal[i];
					
					
				}
				// if there are two or more square that have same possibility, 
				// we will look next move possibilities.
				else if(minPossibility == possibilityBoard[currentRow + vertical[i]]
														 [currentColumn + horizontal[i]])
				{
					...
					int a = nextMin(minPossibleRow, minPossibleColumn);
				    int b = nextMin(currentRow + vertical[i], currentColumn + horizontal[i]);
					if(a > b)
					{
						// move decision
						moveNumber = i;
	...
	private static int nextMin(int row, int column)
	{
		int minPossibility = 9;
		...
		{
			if(row + vertical[i] >= 0 && 
					column + horizontal[i] >= 0 &&
						row + vertical[i] < 8 && 
							column + horizontal[i] < 8 &&
								possibilityBoard[row + vertical[i]]
												[column + horizontal[i]] > 0)
			{	
				if(minPossibility > possibilityBoard[row + vertical[i]]
														[column + horizontal[i]])
				{
					minPossibility = possibilityBoard[row + vertical[i]]
													 [column + horizontal[i]]; 
				}
			}
		}
